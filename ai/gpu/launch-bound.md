...
`__launch_bounds__(maxThreads, minBlocksPerSM)` is a hint in CUDA for compiler to tell it:
1. maxThreads: The max threads number in a block.
2. minBlocksPerSM: (optional) The minimum number of a SM could be able to run simutaneously.

### Expection
We do expect these hint could help compiler do more optimization to help improve performance, but it didn't always bring benefits.
...
__global__ void __launch_bounds__((BM * BN) / (TM * TN), 1)
  ...
  __shared__ __align__(16) data_type s_b[BK * BN_WO_BANK_CONFLICT];
...
it assign a launch bound **((BM * BN) / (TM * TN), 1)**, but if I remove the optional parameter for minimum number of a SM could run (**1** in this case), we could see about **5%** gain!